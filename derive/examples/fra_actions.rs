...
use std::str::FromStr;
use bitcoincore_rpc::{Auth, Client, RpcApi, rpc_json::ListUnspentResultEntry};
...
fn run_single_signer_test(
    ...
    // 1. 构建脚本和地址
    let script_bytes = build_fra_script(action).to_bytes();
...
fn test_fra_transfer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let action = FraAction::Transfer {
        asset_id: [1; 32],
        amount: 1000,
        receiver: XOnlyPk::from(receiver_pk),
        sender: XOnlyPk::from(sender_pk),
    };

    let script = ScriptBuf::from(build_fra_script(action).to_bytes());
    let builder = TaprootBuilder::new().add_leaf(0, script.clone()).unwrap();
    let spend_info = builder.finalize(&env.secp, internal_pk).unwrap();
    let address = Address::p2tr(&env.secp, internal_pk, spend_info.merkle_root(), Network::Regtest);

    let funding_txid = env.rpc.send_to_address(&address.clone().assume_checked(), Amount::from_sat(50000), None, None, None, None, None, None)?;
    let funding_addr = env.rpc.get_new_address(None, None)?.assume_checked();
    env.rpc.generate_to_address(1, &funding_addr)?;
    println!("\nAction funded with TXID: {}", funding_txid);
    ...
    let mut sighasher = SighashCache::new(&spend_tx);
    let leaf_hash = taproot::TapLeafHash::from_script(&script, LeafVersion::TapScript);
    let sighash = sighasher.taproot_script_spend_signature_hash(0, &Prevouts::All(&[TxOut { value: prevout_value, script_pubkey: address.script_pubkey() }]), leaf_hash, sighash::TapSighashType::Default)?;
    let msg = Message::from(sighash);
...
fn test_fra_mint() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (_, receiver_out_pk) = env.receiver_kp.public_key().x_only_public_key();

    let action = FraAction::Mint {
        asset_id: [2; 32],
        amount: 500,
        receiver: OutputPk::from_x_only_pk(XOnlyPk::from(receiver_pk), receiver_out_pk),
        minter: XOnlyPk::from(minter_pk),
    };
...
fn test_fra_rollback() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let action = FraAction::Rollback {
        ...
        amount: 150,
        owner: XOnlyPk::from(owner_pk),
        minter: XOnlyPk::from(minter_pk), // The target to roll back to
    };
    run_single_signer_test(action, &env.sender_kp)
}
...
fn test_fra_freeze() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let action = FraAction::Freeze {
        asset_id: [6; 32],
        authority: XOnlyPk::from(authority_pk),
    };
...
fn test_fra_grant_role() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let action = FraAction::GrantRole {
        ...
        target: XOnlyPk::from(target_pk),
        admin: XOnlyPk::from(admin_pk),
    };
...
// 注意: Split 和 Merge 的测试稍微复杂，因为它们需要在链上验证状态。
// 这里的测试主要验证脚本的签名逻辑是否正确。
// 一个完整的 Split/Merge 测试需要构造一个包含多个输入的交易（Merge）
// 或多个输出的交易（Split），并确保 sighash 正确覆盖了所有组件。
...
fn test_fra_split() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let env = setup_test_environment()?;
    let (owner_pk, _) = env.sender_kp.x_only_public_key();
    let (_, out_pk_par) = env.receiver_kp.public_key().x_only_public_key();

    let action = FraAction::Split {
        asset_id: [12; 32],
        orig_amount: 1000,
        outputs: vec![
            (600, OutputPk::from_x_only_pk(XOnlyPk::from(owner_pk), out_pk_par)),
            (400, OutputPk::from_x_only_pk(XOnlyPk::from(owner_pk), out_pk_par)),
        ],
        owner: XOnlyPk::from(owner_pk),
    };
    run_single_signer_test(action, &env.sender_kp)
...
fn test_fra_merge() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let env = setup_test_environment()?;
    let (owner_pk, _) = env.sender_kp.x_only_public_key();
    let (recipient_pk, recipient_parity) = env.receiver_kp.x_only_public_key();

    let action = FraAction::Merge {
        asset_id: [13; 32],
        inputs: vec![300, 700],
        recipient: OutputPk::from_x_only_pk(XOnlyPk::from(recipient_pk), recipient_parity),
        owner: XOnlyPk::from(owner_pk),
    };
    run_single_signer_test(action, &env.sender_kp)
}
