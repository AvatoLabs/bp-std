...
///
/// ## 修正说明
/// 原来的 `wrapping_add` 写法依赖于 `TapCode` 枚举的具体数值，不够健壮。
/// 对于 1-75 字节的推送，操作码就是其长度本身。
/// 此处改为直接使用 `len` 作为操作码，这是标准且可靠的实现。
...
/// 构造对应操作的 TapScript (为所有单签操作添加了堆栈清理)
pub fn build_fra_script(action: FraAction) -> TapScript {
    let mut buf = Vec::new();
    match action {
        // --- Transfer: 本身就是有效的，保持不变 ---
...
/// 将一系列 FRA 动作打包成 Taproot 解锁证明 (保持不变)
